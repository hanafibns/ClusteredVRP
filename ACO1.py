...
def Longueur_partielle(C,solution):
    ...
    return s
def construction(n,toij,nij,alpha,beta,c,k,q0,Q,p):
    solutions=[]
    for i1 in range(k):
        ...
        while(n1!=1):
            ...
            if(q<=q0):
                ...
                j = v.index(a)
                #deltatoijk=[[0 for i in range(n)] for j in range(n)]
                #for i in range(n):
                #    for j in range(n):
                #        if(i!=j):
                #            deltatoijk[i][j]=1/(NN(n,c)*n)
                #for i in range(n):
                #    for j in range(n):
                #        toij[i][j]=(1-p)*(toij[i][j])+(p)*deltatoijk[i][j]
        ...
        deltatoijk=[[0 for i in range(n)] for j in range(n)]
        for i in range(n-1):
                deltatoijk[chemin[i]][chemin[i+1]]=1/(NN(n,c)*n)
        for i in range(n):
            toij[chemin[i]][chemin[i+1]]=(1-r)*(toij[chemin[i]][chemin[i+1]])+(r)*deltatoijk[chemin[i]][chemin[i+1]]
...
def colonie_de_fourmis(n,toij,nij,alpha,beta,c,k,q0,N,Q,p):
    Solutions=construction(n,toij,nij,alpha,beta,c,k,q0,Q,p)
    Longueurs=[]
    for i in range(len(Solutions)):
        Longueurs.append(Longueur(n,c,Solutions[i]))
    ...
    while(Nit<N):
        ...
        #print("MAJ toij = ",toij)
        toij1=toij
        Solutions=construction(n,toij1,nij,alpha,beta,c,k,q0,Q,p)
        Longueurs=[]
        for i in range(len(Solutions)):
            Longueurs.append(Longueur(n,c,Solutions[i]))
        Nit=Nit+1
